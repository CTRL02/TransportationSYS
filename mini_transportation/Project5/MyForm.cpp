...
void loadToFileDirected(Graph* f)
{
    std::ofstream m("graphDirected.txt");
    for (auto x : f->graphDirected)
    {
        m << x.first << ": ";
        for (auto y : f->graphDirected[x.first])
        {
            m << y.first << " " << std::to_string(y.second) << " ";
        }
        m << "\n";
...
int main(array<System::String^>^ args)
{
    ...
    loadToFileDirected(form.graphPtr);
